...
1. **运行期权威表** `ssoptimizer-{platform}.tiny`（入库）：人工热点表。NanoForge R4 起运行期 remap 由 NanoForge + SourceSector 全量表承担，本表仅剩构建期消费者（`generateFullMappings` 合并优先级最高层）。新增/修改语义映射只改这里。
2. **构建期全量表** `ssoptimizer-{platform}-full.tiny`（`mapping/build/generated/`，生成物不入库）：由 `generateFullMappings` 从人工表 + scope 语义片段 + 结构指纹占位名（`C_<hash8>`/`f_<hash8>`/`m_<hash8>`）确定性导出，分层优先级为 占位生成 < identity 片段 < scope 片段 < 人工表。生成层含成员名提升：混淆器未改写的原始成员名（如 `ship`、`render`）直接作为 named 名。供 `remapGameClasspathToNamed` 生成全量 named 编译 classpath。
...
4. `app` 源码只允许引用人工表中的 named 名、或 `ssoptimizer-identity.tiny` 中登记保持原名的游戏类；**禁止引用生成层名称**（占位名 `C_`/`f_`/`m_` 与自动提升的原始名）——占位名与提升名未经人工审核，跨平台一致性不受保证。需要在 `app` 引用的成员，必须先迁入人工表再引用。
